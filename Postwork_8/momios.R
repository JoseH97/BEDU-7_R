...
(Ym)
(Ym[15])#basicamente son los primeros 15 meses en los que hubo partido
places <- which(Ym[15]==format(scores$date, "%Y-%m")) # Consideramos partidos de 15 meses para comenzar a ajustar el modelo
(places) # obtenemos los indices de las filas que contienen partidos en el 2011-12, sin importar el dia
ffe <- scores$date[max(places)] # Fecha final conjunto de entrenamiento
(ffe) # es decir, el ultimo partido de ese del mes de diciembre de 2011

# Consideraremos partidos de 15 meses para comenzar a ajustar el modelo. 
# Así, nuestro primer conjunto de entrenamiento consiste de datos de partidos hasta el `r ffe` 
# es decir, todos los partidos que se jugaron antes del 2011-12-18 = ffe. en indices de filas seria de la fila 1 a la 540 para el conjunto de entrenamiento

train <- scores %>% filter(date <= ffe)
test <- scores %>% filter(date > ffe) #los que restan del total del conjunto

#lo anterior dicho se puede entender mejor con la ejecucion de las sig. lineas
head(train, n = 1); tail(train, n = 1)
head(test, n = 1); tail(test, n = 1)

# Primer ajuste del modelo

traindate <- unique(train$date) #nos trae todos las fechas en las que hubo al menos un partido de el conjunto train
...
# volvemos a definir las variables anteriores y por tanto, antes hay que borrarlas con NULL
phs <- NULL; pas <- NULL; pht <- NULL; pat <- NULL
# hacemos lo de haya arriba pero más chido??? creo que sí
...
p <- ggplot(g, aes(x=Num_Ap, y=Capital)) + geom_line( color="darkslategray4") + geom_point() +
  ggtitle ("Realizando una secuencia de juegos") + # Título del gráfico+
  theme (plot.title = element_text(family="Times New Roman",
                                   size=12, #Tamaño de la letra del título
                                   vjust=2, #Justificación vertical, para separarlo del gráfico
                                   face="bold", #Letra negrita
                                   color="darkslategray"))+ #Color del texto
  labs(x = "Número de juegos",y = "Capital") + # Etiquetas o títulos de los ejes
...
g <- data.frame(Num_Ap = 1:length(g), Capital = g)
p <- ggplot(g, aes(x=Num_Ap, y=Capital)) + geom_line( color="darkslategray3") + geom_point() +
  ggtitle ("Realizando una secuencia de juegos") + # Título del gráfico
  theme (plot.title = element_text(family="Times New Roman",
                                   size=12, #Tamaño de la letra del título
                                   vjust=2, #Justificación vertical, para separarlo del gráfico
                                   ...
                                   color="darkslategray"))+ #Color del texto
  labs(x = "Número de juegos",y = "Capital") + # Etiquetas o títulos de los ejes
  theme(axis.title = element_text(face="bold", family="Times New Roman", colour="darkslategray")) + # Formato de los títulos de los ejes
  theme(axis.text.x = element_text(face = "bold", color="darkslategray4", size = 10, hjust = 1),
        axis.text.y = element_text(face = "bold", color="darkslategray4", size = 10, hjust = 1))  
p
ggplotly(p)
